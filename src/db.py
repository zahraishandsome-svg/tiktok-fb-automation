...
def upsert_channel(channel_cfg: Dict[str, Any]) -> None:
    ...
    with conn:
        conn.execute("""
            INSERT INTO channels (id, tiktok_username, facebook_page_name, enabled, updated_at)
            VALUES (:id, :tiktok_username, :facebook_page_name, :enabled, :now)
        ...
        """, {
            ...
            "facebook_page_name": channel_cfg["facebook_page_name"],
...
def mark_uploaded(channel_id: str, tiktok_video_id: str, fb_video_id: str) -> None:
    conn = get_connection()
    with conn:
        conn.execute("""
